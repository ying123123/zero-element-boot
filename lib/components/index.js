import NamedLayout from "./NamedLayout";
import NamedList from "./NamedList";
import NamedCart from "./NamedCart";
import NamedContainer from "./NamedContainer";
import NamedSeperator from "./NamedSeperator";
import NamedGateway from "./NamedGateway"; // AutoComponent

import AutoComponent from "./AutoComponent";
import AutoComponentL2 from "./AutoComponentL2"; // default to export core components

export { NamedLayout, NamedList, NamedCart, NamedContainer, NamedSeperator, NamedGateway // AutoComponent
, AutoComponent, AutoComponentL2 };